...
def create_sales_orders_from_fishbowl_data(
    session: Session, orm: ORM, fb_open_sales_orders: list[FBSalesOrder]
) -> None:
    ...
    for fishbowl_sales_order in fb_open_sales_orders:
        backend_logger.info(f"Processing sales order {fishbowl_sales_order.number}")
        ...
        for fishbowl_sales_order_item in fishbowl_sales_order.items:
            ...
            if parent_part in child_parts:
                ...
                child_parts[
                    parent_part
                ] = fb_child_parts = fb_utilities.get_child_parts(orm, parent_part)
...
def update_unfinished_sales_orders(session: Session, orm: ORM) -> None:
    for sales_order in SalesOrder.find_all_unfinished():
        fb_sales_order = fb_models.FBSalesOrder.find_by_number(orm, sales_order.number)
