...
class Customer(Base, Auditing):
    ...
    def find_by_name(name: str) -> Customer:
        ...
        return session.query(Customer).filter(Customer.name == name).one()
    ...
    @staticmethod
    def create_from_fishbowl_sales_orders(fishbowl_sales_orders: list) -> None:
        """Creates a customer from the given sales orders."""
        s = time.perf_counter()
        created = 0
        customer_names = [customer.name for customer in Customer.find_all(session)]
    ...
    @staticmethod
    def from_fishbowl_customer(fishbowl_customer) -> Customer:
