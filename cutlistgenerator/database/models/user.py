...
from cutlistgenerator.database import Base, session
...
class User(Base):
    ...
    def create_default_data() -> None:
        ...
        for user in data:
            x = session.query(User).filter(User.username == user.username).first()
            if x:
                continue
            session.add(user)
        session.commit()
...
class UserLoginEventType(Base):
    ...
    def find_by_name(name: str) -> UserLoginEventType:
        ...
        return session.query(UserLoginEventType).filter_by(name=name).first()

    @staticmethod
    def find_by_id(id: int) -> UserLoginEventType:
        """Find a user login log event type by id."""
        return session.query(UserLoginEventType).filter_by(id=id).first()

    @staticmethod
    def find_all() -> list[UserLoginEventType]:
        """Find all user login log event types."""
        return session.query(UserLoginEventType).all()
    ...
    def create_default_data() -> None:
        ...
        for event_type in data:
            x = (
                session.query(UserLoginEventType)
...
class UserProperty(Base):
    ...
    def find_by_user_id(user_id: int) -> list[UserProperty]:
        ...
        return session.query(UserProperty).filter_by(user_id=user_id).all()

    @staticmethod
    def find_by_user_id_and_property_name(user_id: int, name: str) -> UserProperty:
        """Find a user property by user id and property name."""
        property = (
            session.query(UserProperty).filter_by(user_id=user_id, name=name).first()
        )
        if not property:
