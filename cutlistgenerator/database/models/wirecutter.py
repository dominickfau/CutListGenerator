...
class WireSize(Base):
    ...
    def find_by_name(name: str) -> WireSize:
        ...
        return session.query(WireSize).filter(WireSize.name == name).first()

    @staticmethod
    def create_default_data():
        """Creates the default data for the database."""
        backend_logger.info("Creating default data for WireSize")
        data = [
        ...
        for item in data:
            x = session.query(WireSize).filter(WireSize.name == item.name).first()
            if x:
                continue
            session.add(item)
        session.commit()
...
class WireCutter(Base, Auditing):
    ...
    def find_by_name(name: str) -> WireCutter:
        ...
        return session.query(WireCutter).filter(WireCutter.name == name).first()

    @staticmethod
    def find_by_id(id: int) -> WireCutter:
        """Returns the wire cutter with the given id."""
        return session.query(WireCutter).filter(WireCutter.id == id).first()
