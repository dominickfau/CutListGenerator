...
from PyQt5.QtCore import QRunnable, QThreadPool, pyqtSlot, pyqtSignal, QObject, Qt, QSettings
...
class MainWindow(Ui_MainWindow, QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.fishbowl_database = FishbowlDatabaseConnection(connection_args=self.fishbowl_credentials)
        self.cut_list_generator_database = MySQLDatabaseConnection(connection_args=self.cutlist_credentials)

        # This is to make sure all system properties exist.
        utilities.create_default_system_properties(self.cut_list_generator_database, self.logger)
    ...
    def configure_settings(self):
        ...
        self.database_setup_completed = utilities.to_bool(self.settings.value("database_setup", False))
        ...
        self.cutlist_credentials['password'] = self.settings.value("password", "Letmein2021")
        self.cutlist_credentials['database'] = self.settings.value("database", "cutlistgenerator")
        self.settings.endGroup()
        ...
        self.fishbowl_credentials['password'] = self.settings.value("password", "fishing")
    ...
    def load_cut_job(self, cut_job: CutJob):
        ...
        if dialog.exec():
            dialog.cut_job.save()
            self.logger.info(f"[CUT JOB] Cut job data saved. Job ID: {dialog.cut_job.id}")
            if dialog.cut_job.is_cut and dialog.cut_job.related_sales_order_item:
                self.logger.info(f"[CUT JOB] Checking if sales order item ID {dialog.cut_job.related_sales_order_item.id} is fully cut.")
                sales_order_item = dialog.cut_job.related_sales_order_item
                if sales_order_item.cut_in_full:
                    self.logger.info(f"Sales order item ID {sales_order_item.id} is fully cut.")
                    sales_order_item.save()
                else:
                    self.logger.info(f"Sales order item ID {sales_order_item.id} is not fully cut.")
        self.thread_get_so_table_data()
    ...
    def get_so_table_data(self, progress_signal=None, progress_data_signal=None) -> List[dict]:
        ...
        cut_list_generator_database = MySQLDatabaseConnection(connection_args=self.cutlist_credentials)
        table_data = []
    ...
    def thread_get_current_fb_data(self):
        ...
        if self.updating_fishbowl_data:
            self.logger.warning("Attempted to get data while fishbowl data was already being updated. Ignoring request.")
            return

        self.updating_fishbowl_data = True

        self.logger.info("Starting thread to get current sales order data from Fishbowl.")
        self.action_fishbowl_Get_Sales_Order_Data.setEnabled(False)

        fishbowl_database = FishbowlDatabaseConnection(connection_args=self.fishbowl_credentials)
        ...
        worker.signals.finished.connect(lambda: self.logger.info("[FISHBOWL DATA] Finished retreiving data from Fishbowl."))
    ...
    def thread_get_so_table_data(self):
        ...
        if self.updating_table:
            self.logger.warning("Attempted to get SO data while table was still updating. Ignoring request.")
            return
        ...
        worker.signals.finished.connect(self.reset_progress_bar)
        worker.signals.finished.connect(lambda: self.logger.info("[TABLE RELOAD] Finished reloading data into table."))
